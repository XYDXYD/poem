...
24);
    ...
    public class LoginAndRegister
    {
        ...
        public void Register(CallBack back)
        {
            registerCallBack = back;

            WebClient registerClient = new WebClient();
            registerClient.Encoding = Encoding.UTF8;
            registerClient.DownloadStringCompleted += new DownloadStringCompletedEventHandler(registerClient_DownloadStringCompleted);
            registerClient.DownloadStringAsync(new Uri("http://1.winphonexiang.sinaapp.com/guwen/login_and_register.php?type=register&user_name=" + _userName + "&password=" + MD5.GetMd5String(_password)));

            //备份到冗余服务器
            WebClient registerClient2 = new WebClient();
            registerClient2.Encoding = Encoding.UTF8;
            registerClient2.DownloadStringCompleted += new DownloadStringCompletedEventHandler(registerClient2_DownloadStringCompleted);
            registerClient2.DownloadStringAsync(new Uri("http://appdev.sysu.edu.cn/~mad/10389233/shici/login_and_register.php?type=register&user_name=" + _userName + "&password=" + MD5.GetMd5String(_password)));
            
        }

        void registerClient2_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            //什么都不做
        }

        void registerClient_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                MessageBox.Show("网络故障，请重试");
            }
            else
            {
                if (e.Result == "duplication")
                {
                ...
                else if (e.Result.IndexOf("success") != -1)
                {
        ...
        public void Login(CallBack back, bool auto = false)
        {
            loginCallBack = back;

            _autoLogin = auto;
            WebClient loginClient = new WebClient();
            loginClient.Encoding = Encoding.UTF8;
            loginClient.DownloadStringCompleted += new DownloadStringCompletedEventHandler(loginClient_DownloadStringCompleted);
            loginClient.DownloadStringAsync(new Uri("http://1.winphonexiang.sinaapp.com/guwen/login_and_register.php?type=login&user_name=" + _userName + "&password=" + MD5.GetMd5String(_password)));
        }
        ...
        void loginClient_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            if (_autoLogin == false)
            {
                if (e.Error != null)
                {
                    if (App._userManager.LoginNotLinked(_userName, _password))
                    //MessageBox.Show("离线登录成功");
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        App._userManager.LoginLinked(_userName, _password, DateTime.Parse(results[0]), int.Parse(results[5]), int.Parse(results[4]), int.Parse(results[2]), int.Parse(results[3]), results[1]);
            ...
            else
            {
                if (e.Error != null)
                {
                    App._userManager.LoginNotLinked(_userName, _password);
                    //MessageBox.Show("自动离线登录成功");
                    loginCallBack();
                }
                else
                {
